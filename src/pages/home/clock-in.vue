    ...
    <view class="clock_in">
        ...
        <view class="statuses">
            <view class="status-left">
                <view class="status-item"  @tap="changeStatus(true)" :class="{'active-postion':onsite}">Onsite Check in</view>
                <view class="status-slice">|</view>
                <view class="status-item" @tap="changeStatus(false)"  :class="{'active-postion':onsite==false}">Offsite Check In</view>
            </view>
            <view class="history">
        ...
        <view class="content">
            ...
            <view class="map">
                <iframe  class="iframemap" v-if="mapUrl" :src="mapUrl" width="100%" height="150%" style="border: 0;" allowfullscreen
        ...
        getTodayAttendancesTime(){
            getTodayAttendances(this.user.id).then(({data, msg, status})=>{
                ...
                if(status == 1){ 
                    ...
                    if (length > 0){
                        if (data[length-1].signOutTime != null && data[length-1].signOutTime != "" ){ 
                            this.isClockedIn = false 
                            this.clockInTime = data[length-1].signInTime.split(" ")[1]
                            uni.setStorageSync("scheduleIn", this.clockInTime) 
                            this.checkOutTime = data[length-1].scheduleSignOutTime 

                        }   else if (data[length-1].signInTime != null && data[length-1].signInTime != "" ){ 
...
<style scoped lang="scss" src="./clock-in.scss">
